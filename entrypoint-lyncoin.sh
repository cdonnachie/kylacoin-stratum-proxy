...
BOOTSTRAP_BASE_URL="https://downloads.lyncoin.com/blockchain-snapshots"
DATA_DIR="/home/lyncoin/.lyncoin"

# Function to log with timestamp (explicitly to stderr)
log() {
    echo "[$(date +'%Y-%m-%d %H:%M:%S')] LCN Bootstrap: $1" >&2
}
...
needs_bootstrap() {
    ...
    if [ -d "$blocks_dir" ]; then
        ...
        if [ $age_days -gt $BOOTSTRAP_MAX_AGE_DAYS ]; then
            log "Blockchain data is $age_days days old (max: $BOOTSTRAP_MAX_AGE_DAYS)"
            return 0
        else
...
get_latest_bootstrap() {
    ...
    local latest_file=$(curl -s --max-time 30 "$BOOTSTRAP_BASE_URL/" 2>&1 | \
                       grep -oE 'lyncoin-[0-9]{4}-[0-9]{2}-[0-9]{2}\.tar\.gz' | \
                       sort -V | tail -1 | tr -d '\n' | tr -d '\r' | xargs)
    ...
    if [ -z "$latest_file" ]; then
        ...
        for date in "$today" "$yesterday" "$two_days_ago"; do
            local candidate="lyncoin-${date}.tar.gz"
...
download_bootstrap() {
    local filename="$1"
    local temp_dir="/tmp/lcn_bootstrap_$$"
    ...
    log "Downloading bootstrap: $filename (~950MB, this may take a while...)"
...
extract_bootstrap() {
    ...
    local backup_dir="/tmp/lcn_backup_$$"
    ...
    # Fix ownership
    chown -R lyncoin:lyncoin "$DATA_DIR"
...
zmqpubhashblock=tcp://0.0.0.0:${LCN_ZMQ_PORT:-29433}
zmqpubrawblock=tcp://0.0.0.0:${LCN_ZMQ_RAW_PORT:-29434}
